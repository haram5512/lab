            ...
            for i,d in enumerate(dets):
                ...
                for box,cls in zip(batch["bboxes"][batch["batch_idx"]==i],batch["cls"][batch["batch_idx"]==i]):
                    if int(cls.item())==0:
                        ...
                        letterbox_box=torch.tensor([[(x-w/2)*size,(y-h/2)*size,(x+w/2)*size,(y+h/2)*size]])
                        gt.append(inverse_letterbox_xyxy(letterbox_box,batch["original_size"][i],batch["letterbox_scale"][i],batch["letterbox_pad"][i])[0])
...
def _coco_metrics(annotations, dataset, preds, gts, matched):
    ...
    gt_count=sum(len(v) for v in gts.values()); tp=sum(len(v) for v in matched.values())
